...
We further note that the parity check matrix $\mat{P}$ defines the 
...
\greg{The memory costs are very high here. For our 128-bit security aggressive
parameters the attack requires $2^{113}$ memory ignoring polynomial factors. We
don't have a standard way of equating time and memory costs, in order to say
that an attack with cost $2^{113}$ time and memory does not break 128-bit
security, since the memory cost is too high.  But we can point out these high
memory costs to help justify the security of the aggressive parameters. If there
was consensus in the crypto community that (say) that an attack having $2^{96}$ memory cost
 does not break 128-bit security, then we could set $m=340$. Or alternatively we could say that the agressive
parameters have more margin when better accounting for the cost of memory. }
...
% \greg{Notation: In Preliminaries, the definition of wPRF gives the attacker $l$ samples, indexed
% with subscripts (the superscript notation is used for secret shares).}
...
\greg{Is this the reason why the \ttwPRF is a weak PRF and not a PRF? I don't think we actually say anywhere
in the paper why it's only a PRF for random inputs, leaving open the possibility that the \ttwPRF is actually a PRF.}
\yuval{This is discussed in the TCC '18 paper, but indeed a brief explanation here can be helpful.}
...
\greg{Does this automatically imply that the usual iterated PRG construction for longer outputs is secure? I'm thinking of the one where
the $n$ bit seed is expanded to $2n$ bits, half are output and half are used as the state/seed for the next iteration.
(in practice we'd probably want a $2s$-bit state)}
