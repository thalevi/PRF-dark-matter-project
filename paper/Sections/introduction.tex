...
We introduce several candidate constructions for (weak) PRF, OWF, and PRG, all based on the principle of alternating between modulo 2 and modulo 3 linear mappings.
...
\subsubsection{Distributed protocols and optimized implementations.}
 ...
 \yuval{Start with a sharp statement on how we improve the online complexity of the wPRF protocols from~\cite{boneh2018-darkmatter}  (both communication and rounds). Then say that this is achieved via a unified approach that applies also to the other primitives we design. This approach is explained below.}
...
\paragraph{Eliminating preprocessing.} All of the 2PC protocols presented in~\cite{boneh2018-darkmatter} rely on preprocessing.
