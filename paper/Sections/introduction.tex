...
and recently analyzed in~\cite{CheonCKK20}. 
...
(which is fixed to the vector $\vec{1}$).
...
It is important to emphasize that~\cite{CheonCKK20} analyzed constructions where the output length is $t=1$,
...
forcing $t \ll m$.
Our security analysis of the candidate wPRF and PRG constructions shows that they resist such simple linear algorithm attacks. 
...
the OWF in Picnic (an instance of the LowMC block cipher~\cite{albrecht2015-lowmc}, which is assumed to be a OWF),
...
Compared to a standard DDH-based OPRF~\cite{jarecki2014-ddhoprf,jarecki2016-ddhoprf}, which require 512 bits of communication for $128$-bit security, our protocol requires slightly higher communication but has much better performance. Our implementation shows that our OPRF protocol is faster than a \textit{single} scalar multiplication over the Curve25519 elliptic curve. Consequently, we expect our protocol to be faster than a number of OPRF protocols~\cite{freedman2005-oprf,jarecki2009-oprf} that are based on algebraic PRFs and require expensive exponentiations.\greg{Replace ``expensive exponentiations'' with ``XX or more exponenetiations'' where XX is the number these protocols need.} In another direction, recent works~\cite{grassi2016-mpcfriendly,seres2021-legendre} construct an OPRF protocol from the Legendre PRF~\cite{damgard1988-legendre}. For 128-bit security and only a {\em single} output bit, the recent protocol from~\cite{seres2021-legendre} has online communication cost of $13$KB, substantially higher than ours (with 128 output bits), and with a higher computational cost.
