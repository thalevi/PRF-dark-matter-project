...
A primary motivation for the alternating moduli paradigm was its MPC-friendliness. Indeed, several MPC protocols were proposed in~\cite{boneh2018-darkmatter}. These protocols were shown to have significant advantages over ones that apply to earlier MPC-friendly designs, but this was argued either for the honest-majority 3-party case or for 2PC in the preprocessing model. 

Another, very different, motivation is the goal of identifying simple function classes that are ``hard to learn.''  Indeed,  the conjectures from~\cite{boneh2018-darkmatter} imply hardness of learning results for low complexity functions such as (depth-2) $\mathsf{ACC}^0$ circuits, sparse $\Z_3$ polynomials, or width-3 branching programs. These conjectures are of interest outside the field of cryptography~\cite{complexity-theory-works}, which further motivates cryptanalysis efforts.

\paragraph{Remaining challenges.} The initial works of~\cite{boneh2018-darkmatter,adventures} have only scratched the surface of the kind of questions one may ask. 
\begin{itemize}
\item
...
\begin{itemize}
    ...
    Let $\mat{K}$ be a secret key matrix in $\Z_2^{m \times n}$. For an input $x$, the $(2,3)$-wPRF candidate is defined as $\mathsf{F}_\mat{K}(x) = \mat{B}(\mat{K}x)$ where $\mat{B} \in \Z_3^{t \times m}$ is a public matrix and $w = \mat{K}x$ is first reinterpreted as a 0/1 vector over $\Z_3$. \yuval{Describe LPN-friendly PRFs here. Maybe make this the last bullet, so that we can start with the new stuff?}
...
\yuval{Give a concrete bottom line about the recommended choice of parameters for the OWF, PRG, wPRF primitives described above. Can include necessary disclaimers. }

\yuval{Highlight the fact (?) that for OWFs and PRGs we can have the best possible input and output size. Motivate this in the context of applications. Contrast with the fact that for wPRF we lose a factor of 2 in input and key size compared to, say, AES. }



\subsubsection{Distributed protocols and optimized implementations.}
 
 \yuval{Start with a sharp statement on how we improve the online complexity of the wPRF protocols from~\cite{boneh2018-darkmatter}  (both communication and rounds). Give some concrete data points for fully distributed PRF, OPRF. Then say that this is achieved via a unified approach that applies also to the other primitives we design. This approach is explained below.}
